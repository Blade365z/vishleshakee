...
export const getNetworkTweetIDs = async (query, from = null, to = null, rangeType, filter = null, isDateTimeAlready = 0,hashtags, mentions) => {
    let dataArgs;
    if (from != null && to != null && isDateTimeAlready == 0) {
        dataArgs = JSON.stringify({ from, to, query, rangeType, filter, isDateTimeAlready,hashtags, mentions });
    } else if (isDateTimeAlready == 1) {
        dataArgs = JSON.stringify({ from, to, query, rangeType, filter, isDateTimeAlready,hashtags, mentions });
    }
    let response = await fetch('UA/getNetworkTweetIDs', {
        method: 'post',
        headers: HeadersForApi,
        body: dataArgs
    })
